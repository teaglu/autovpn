...
VOID Log::log(const SLogInfo &sInfo, LPCTSTR format, ...)
{
	bool doLog = true;

#ifndef _DEBUG
	if (sInfo.eLevel == LL_DEBUG) {
		doLog = false;
	}
#endif
	...
	if (doLog) {
		...
		if ((offset = message.Find(_T("{w32err}"))) != -1) {
			...
			if (size == 0) {
				_sntprintf_s(win32Err, 1023, _T("Unknown Win32 Error Code: %08X"), lError);
			}
